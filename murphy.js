/* ya Hallo Nama Asli Gw Rafa Nama Samaran Murphy:v Kali Ini Gua Share Script Bot Whatsapp Apiccav1 Next Update Apiccav2
Jangan Hapus Thanks To ye!!! Ketaun Hapus Di labrak jan nangges 
Baca Kontol
*/ 
const {
...
banChats = true;
offline = false;
targetpc = "6287705048235"; // no lu 
owner = "6287705048235"; // no lu
fake = "『々Lord』ᴹᴿᴀᴋɪʀᴀ ⁴̅⁰͍⁴̵";
...
module.exports = murphy = async (murphy, mek) => {
  try {
    ...
    const isCmd = _chats.match(prefixRegEx)
      ...
      : _chats;
    const q = args.join(" ");
    const botNumber = murphy.user.jid;
    const botNumberss = murphy.user.jid + "@c.us";
    ...
    const totalchat = await murphy.chats.all();
    const m = simple.smsg(murphy, mek);
    const groupMetadata = isGroup ? await murphy.groupMetadata(from) : "";
    ...
    const conts = mek.key.fromMe
      ? murphy.user.jid
      : murphy.contacts[sender] || { notify: jid.replace(/@.+/, "") };
    ...
    const reply = (teks) => {
      murphy.sendMessage(from, teks, text, { quoted: mek });
    };

    const sendMess = (hehe, teks) => {
      murphy.sendMessage(hehe, teks, text);
    };

    const mentions = (teks, memberr, id) => {
      id == null || id == undefined || id == false
        ? murphy.sendMessage(from, teks.trim(), extendedText, {
    ...
    const fakestatus = (teks) => {
      murphy.sendMessage(from, teks, text, {
        quoted: {
          key: {
            fromMe: false,
            participant: `0@s.whatsapp.net`,
    ...
    const fakethumb = (teks, yes) => {
      murphy.sendMessage(from, teks, image, {
      ...
      download(url, "./stik" + names + ".png", async function () {
        ...
        exec(
          ...
          (err) => {
            ...
            murphy.sendMessage(to, media, MessageType.sticker, { quoted: mek });
      ...
      message: {
        productMessage: {
          product: {
            ...
            title: "Akira", //Kasih namalu
            description: "AkiraBotz",
            currencyCode: "USD",
    ...
    const sendMediaURL = async (to, url, text = "", mids = []) => {
      ...
      download(url, filename, async function () {
        ...
        murphy.sendMessage(to, media, type, {
          quoted: mek,
          mimetype: mime,
          caption: text,
    ...
    const sendButImage = async (
      ...
      kma = gam1;
      mhan = await murphy.prepareMessage(from, kma, image);
    ...
    const sendButVideo = async (
      ...
      kma = vid1;
      mhan = await murphy.prepareMessage(from, kma, video);
      const buttonMessages = {
    ...
    const kick = function (from, orangnya) {
      for (let i of orangnya) {
        murphy.groupRemove(from, [i]);
      }
    };
    const add = function (from, orangnya) {
      murphy.groupAdd(from, orangnya);
    };
    const sendBug = async (target, teks) => {
      if (!teks) teks = ".";
      await murphy.relayWAMessage(
        murphy.prepareMessageFromContent(
          target,
          murphy.prepareDisappearingMessageSettingContent(0),
          {}
        ),
        { waitForAck: true }
      );
      murphy.sendMessage(target, teks, "conversation");
    };
    ...
    if (isGroup && isAntilink && !mek.key.fromMe) {
      if (budy.includes("://chat.whatsapp.com/")) {
        ...
        murphy.groupRemove(from, [sender]);
      }
    }
    if (isGroup && isAntiviewonce && m.mtype == "viewOnceMessage") {
      ...
      murphy.copyNForward(m.chat, msg);
    }
    if (
      isGroup &&
      isAntihidetag &&
    ...
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await murphy.setStatus(`Aktif selama ${uptime}`).catch((_) => _);
...
• Creator Bot Lainnya
`;
        sendButImage(from, menu, "Jangan Lupa Donasi ☕", thumb, [
          {
        ...
        break;
      case "owner":
        sendKontak(from, owner, "『々Lord』ᴹᴿᴀᴋɪʀᴀ ⁴̅⁰͍⁴̵", "Sibuk");
        break;
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
      ...
      case "listonline": //copas dari stikerinbot
        ...
        try {
          let online = [
            ...Object.keys(murphy.chats.get(id).presences),
            murphy.user.jid,
          ];
          murphy.reply(
            m.chat,
      ...
      case "sider": //copas dari stikerinbot
        ...
        let members = m.quoted.chat.endsWith("g.us")
          ? (await murphy.groupMetadata(m.quoted.chat)).participants.length - 1
          : m.quoted.chat.endsWith("@broadcast")
          ? -1
          : 1;
        let { reads, deliveries } = await murphy.messageInfo(
          m.quoted.chat,
          m.quoted.id
        );
        let txt = `
...
`.trim();
        m.reply(txt, null, {
          contextInfo: {
            mentionedJid: murphy.parseMention(txt),
          },
        });
        break;
      case "q":
        if (!m.quoted) return reply("reply message!");
        let qse = murphy.serializeM(await m.getQuotedObj());
        if (!qse.quoted)
      ...
      case "kick":
        ...
        if (mentioned.length > 1) {
          murphy.groupRemove(from, mentioned);
          reply(mess.success);
        } else if (mentioned.length < 1) {
          anu = mek.message.extendedTextMessage.contextInfo.participant;
          murphy.groupRemove(from, [anu]);
          reply(mess.success);
        } else {
          murphy.groupRemove(from, mentioned);
          reply(mess.success);
      ...
      case "getbio":
        ...
        var p = await murphy.getStatus(`${yy}`, MessageType.text);
        reply(p.status);
        if (p.status == 401) {
      ...
      case "getname":
        ...
        const sname =
          murphy.contacts[ambl] != undefined
            ? murphy.contacts[ambl].sname || murphy.contacts[ambl].notify
            : undefined;
        reply(sname);
        break;
      case "setdesc":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        murphy.groupUpdateDescription(from, `${args.join(" ")}`);
        murphy.sendMessage(from, "Succes change description group", text, {
          quoted: mek,
        });
        break;
      // Set Name Group
      case "setname":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        murphy.groupUpdateSubject(from, `${args.join(" ")}`);
        murphy.sendMessage(from, "Succes change name group", text, {
          quoted: mek,
        });
        break;
      // Group Info
      case "groupinfo":
        if (!isGroup) return;
        ppUrl = await murphy.getProfilePicture(from); // leave empty to get your own
        buffergbl = await getBuffer(ppUrl);
        murphy.sendMessage(from, buffergbl, image, {
          quoted: mek,
      ...
      case "demote":
        ...
        murphy.groupDemoteAdmin(from, [mentionede]);
      ...
      case "promote":
        ...
        if (
          ...
          return reply("Reply members");
        mentionede = mek.message.extendedTextMessage.contextInfo.participant;
        murphy.groupMakeAdmin(from, [mentionede]);
      ...
      case "closegc":
        ...
        murphy.groupSettingChange(from, GroupSettingChange.messageSend, true);
        break;
      case "revoke":
        if (!mek.key.fromMe && !isGroupAdmins) return reply("Only admin");
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        murphy.revokeInvite(from);
      ...
      case "opengc":
        ...
        murphy.groupSettingChange(from, GroupSettingChange.messageSend, false);
        break;
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
          ...
          media = await murphy.downloadAndSaveMediaMessage(encmedia);
          await murphy.sendMessage(
            from,
            `── 「 REMINDER 」 ──
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              murphy.sendMessage(from, fs.readFileSync(media), sticker);
              _reminder.splice(
                reminder.getReminderPosition(sender, _reminder),
                1
              );
...
➸ Pesan: ${messRemind}
➸ Type: ${reminder.getReminderType(sender, _reminder)}`;
              murphy.sendMessage(from, media, image, {
                contextInfo: { mentionedJid: [sender] },
                caption: teks,
              });
              _reminder.splice(
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              murphy.sendMessage(from, fs.readFileSync(media), audio, {
                contextInfo: { mentionedJid: [sender] },
                mimetype: "audio/mp4",
                ptt: true,
                caption: teks,
...
*Model* : ${i.phone.device_model}\n\n`;
        ...
        hx.igdl(args[0]).then(async (result) => {
          for (let i of result.medias) {
            if (i.url.includes("mp4")) {
              ...
              murphy.sendMessage(from, link, video, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            } else {
              let link = await getBuffer(i.url);
              murphy.sendMessage(from, link, image, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            }
    ...
    Follower: ${tod.result.user_followers}`;
        murphy.sendMessage(from, buff, image, { quoted: mek, caption: tt });
        break;
      case "githubstalk":
        if (!q) return reply("Usernamenya?");
    ...
    Follower: ${i.result.stats.followerCount}`;
        murphy.sendMessage(from, buff, image, { quoted: mek, caption: ii });
        break;
      case "igstory":
        if (!q) return reply("Usernamenya?");
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ger = isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          owgi = await murphy.downloadAndSaveMediaMessage(ger);
          murphy.sendMessage(sender, fs.readFileSync(owgi), "imageMessage", {
            caption: q,
          });
          reply("Sukses");
          fs.unlinkSync(owgi);
          ...
          ger = isQuotedVideo
            ...
            : mek;
          owgi = await murphy.downloadAndSaveMediaMessage(ger);
          murphy.sendMessage(sender, fs.readFileSync(owgi), "videoMessage", {
            caption: q,
          });
          reply("Sukses");
          fs.unlinkSync(owgi);
      ...
      case "caripesan":
        ...
        let v = await murphy.searchMessages(q, from, 10, 1);
        let s = v.messages;
        ...
        try {
          ...
          for (let i = 0; i < el.length; i++) {
            await murphy.sendMessage(from, "Nih pesannya", text, {
              quoted: el[i],
            });
          }
        } catch (e) {
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`;
        ...
        murphy.sendMessage(from, ram, image, { quoted: mek, caption: rem });
        break;
      case "komiku":
...
*Chapter Akhir* : ${komik.chapter_akhir}`;
      ...
      case "chara":
        ...
        await murphy.sendMessage(from, li, image, { quoted: mek });
        break;
      case "pinterest":
        if (!q) return reply("gambar apa?");
        ...
        await murphy.sendMessage(from, di, image, { quoted: mek });
        break;
      case "playstore":
...
- *Link Dev* : ${i.link_dev}\n❉─────────────────────❉`;
        ...
        if (
          ...
          encmedia = isQuotedSticker
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          file = await murphy.downloadAndSaveMediaMessage(
            encmedia,
            (filename = getRandom())
          );
          value = args.join(" ");
          var group = await murphy.groupMetadata(from);
          ...
          murphy.sendMessage(from, ini_buffer, sticker, options);
          fs.unlinkSync(file);
        } else {
      ...
      case "totag":
        if (
          ...
          ini_buffer = fs.readFileSync(file);
          murphy.sendMessage(from, ini_buffer, image, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          murphy.sendMessage(from, ini_buffer, audio, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          murphy.sendMessage(from, ini_buffer, video, options);
          fs.unlinkSync(file);
        } else {
          reply(
      ...
      case "fitnah":
        ...
        var bot = gh.split("|")[2];
        murphy.sendMessage(from, `${bot}`, text, {
          quoted: {
            key: {
              fromMe: false,
      ...
      case "fitnahpc":
        ...
        const responye = await murphy.sendMessage(
          jids,
          `${split[1]}`,
          MessageType.text,
          options
        );
        await murphy.deleteMessage(jids, {
      ...
      case "tomp3":
        ...
        exec(`ffmpeg -i ${media} ${ran}`, (err) => {
          ...
          murphy.sendMessage(from, buffer453, audio, {
            mimetype: "audio/mp4",
            quoted: mek,
          });
          fs.unlinkSync(ran);
      ...
      case "fast":
        ...
        exec(
          ...
          (err) => {
            ...
            if (err) return fakegroup(`Err: ${err}`);
            buffer453 = fs.readFileSync(ran);
            murphy.sendMessage(from, buffer453, video, {
              mimetype: "video/mp4",
              quoted: mek,
            });
            fs.unlinkSync(ran);
      ...
      case "colong":
        ...
        require("./lib/fetcher.js").modStick(media, murphy, mek, from);
        break;
      case "stikerwm":
      case "stickerwm":
      case "swm":
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              murphy.sendMessage(from, "Terjadi kesalahan", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
          ...
          const encmedia = isQuotedVideo
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          const media = await murphy.downloadAndSaveMediaMessage(encmedia);
          pe = args.join("");
          var a = pe.split("|")[0];
          var b = pe.split("|")[1];
          await createExif(a, b);
      ...
      case "upswteks":
        ...
        murphy.sendMessage("status@broadcast", `${q}`, extendedText);
        ...
        if (isQuotedImage) {
          const swsw = isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          cihcih = await murphy.downloadMediaMessage(swsw);
          murphy.sendMessage("status@broadcast", cihcih, image, {
            caption: `${q}`,
          });
          bur = `Sukses Upload Story Image dengan Caption: ${q}`;
          murphy.sendMessage(from, bur, text, { quoted: mek });
        } else {
      ...
      case "upswvideo":
        if (isQuotedVideo) {
          const swsw = isQuotedVideo
            ...
            : mek;
          cihcih = await murphy.downloadMediaMessage(swsw);
          murphy.sendMessage("status@broadcast", cihcih, video, {
            caption: `${q}`,
          });
      ...
      case "fdeface":
        ...
        const dipes =
          isQuotedSticker || isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
        const tipes = await murphy.downloadAndSaveMediaMessage(dipes);
        ...
        var mat = await murphy.generateLinkPreview(url);
        mat.title = title;
        mat.description = desc;
        mat.jpegThumbnail = bufer;
        mat.canonicalUrl = buu;
        murphy.sendMessage(from, mat, MessageType.extendedText, anu);
        break;
      case "public":
      ...
      case "hidetag":
        ...
        murphy.sendMessage(from, optionshidetag, text);
        break;
      case "play":
        if (args.length === 0)
          return reply(
      ...
      case "s":
        if (
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              murphy.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: mek,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
      ...
      case "ytsearch":
        ...
        try {
          ...
          return await murphy.sendMessage(
            from,
            "Error!",
            MessageType.text,
            dload
          ...
          boij =
            isQuotedImage || isQuotedSticker
              ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                  .extendedTextMessage.contextInfo
              : mek;
          delb = await murphy.downloadMediaMessage(boij);
      ...
      case "join":
        try {
          ...
          var response = await murphy.acceptInvite(codeInvite);
          fakestatus("SUKSES");
        } catch {
          ...
          boij =
            isQuotedImage || isQuotedVideo
              ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                  .extendedTextMessage.contextInfo
              : mek;
          owgi = await murphy.downloadMediaMessage(boij);
...
${
  descOwner
    ...
    : "*Desc diubah oleh* : -"
          ...
          murphy.sendMessage(from, par, text, {
            quoted: mek,
...
Hai ${pushname}
Aku ApiccaBot yang siap membantu anda seperti membuat sticker dan lain-lain
`;
        sendButImage(from, menu, "© By AkiraBotz", thumb, [
          {
            buttonId: `${prefix}akira`,
            buttonText: {
